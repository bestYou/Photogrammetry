...
namespace QFJH.DataStruct
{
    ...
    public class CameraPara
    {
        ...
        /// 像主点位置x
        /// </summary>
        public double MainPosX { get; private set; }

        /// <summary>
        /// 像主点位置y
        /// </summary>
        public double MainPosY { get; private set; }
        ...
        private void Process()
        {
            ...
            {
                if (aLine.Contains("相机参数"))
                {
                    if (aLine.Contains("UltraCam"))
                    {
                        this.Type = "UltraCam";
                        continue;
                    }
                }
                else if (aLine.Contains("图像宽度"))
                {
                ...
                else if (aLine.Contains("像主点位置x"))
                {
                    if (aLine.Contains("毫米"))
                    {
                        this.MainPosX = double.Parse(GetNumberInLine(aLine));
                        continue;
                    }
                }
                else if (aLine.Contains("像主点位置y"))
                {
                    if (aLine.Contains("毫米"))
                    {
                        this.MainPosY = double.Parse(GetNumberInLine(aLine));
                        continue;
                    }
                }
